...
namespace Chaincode.NET.Extensions
{
    public static class ChaincodeFunctionParameterInformationExtensions
    {
        ...
        public static T Get<T>(this Parameters parameters, int index)
        {
            if (index < 0)
            {
                throw new ArgumentException("index can not be less than zero", nameof(index));
            }

            if (index > parameters.Count)
            {
                return default; // TODO: or throw? 
            }
        ...
        ) => info.Parameters.TryGet(index, out convertedValue);
